...
void BubbleSort2(float vec[], char lettere[], int dim)
{
    for (int c = 0; c < dim; c++)
    {
...
void BubbleSort(float vec[], char lettere[], int dim)
{
    for (int c = 0; c < dim; c++)
    {
        for (int i = c + 1; i < dim; i++)
...
int main()
{
    ...
    do
    {
        ...
        for (int i = 0; i < 26; i++)
        {
            val[i] = (float)((float)val[i] / completa.length()) * 100;
        }
        ...
        for (int i = 0; i < 26; i++)
        {
            if (val[i] != 0.0)
            {
                stringstream valore;
                string se;
                valore << fixed << setprecision(2) << val[i];
                se = valore.str();
                
                if (se[se.length() - 1] == '0'&&s[se.length() - 2] != '.')
                {
                    se = se.substr(0, se.length() - 1);
                }/*else if(s[s.length() - 1] == '0' && s[s.length() - 2] == '.'){
                    se = se.substr(0, se.length() - 1);
                }
                */
                fileOutput << lettere[i] << ":" << se << " ";
            }
        }
        fileOutput << "\n";
