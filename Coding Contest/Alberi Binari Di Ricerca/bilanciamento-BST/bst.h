...
public:
    ...
    T CalcolaNodi(BSTNode<T> *ptr)
    {
        int cont = 1;

        if (ptr != nullptr)
        {
            BSTNode<T> *mini = min(ptr);
            BSTNode<T> *maxi = max(ptr);
            if (typeid(ptr->getKey()) == typeid(int))
            {
                while (mini->getKey() != maxi->getKey())
                {
                    cont++;
                    mini = successor(mini);
                }
            ...
            if (typeid(ptr->getKey()) == typeid(double))
            {
                while (mini->getKey() != maxi->getKey())
                {
                    cont++;
    ...
    T BilanciamentoBST(T key)
    {
        BSTNode<T> *aux = search(key);

        T six = CalcolaNodi(aux->left);
        T dex = CalcolaNodi(aux->right);
