...
public:
    ...
    int CalcolaAltezza(T key)
    {
        return CalcolaAltezza(root, key);
    }

    int CalcolaAltezza(BSTNode<T> *ptr, T key)
    {
        int cont = 0;
    ...
    T Predecessore(T min, T max)
    {
        return Predecessore(root, min, max);
    }

    T Predecessore(BSTNode<T> *ptr, T min, T max)
    {
        bool cond = false;
        T condi = ptr->getKey();
        while (ptr != nullptr || ptr->getKey() != min || ptr->getKey() != max)
        {
            if (min > ptr->getKey() && max > ptr->getKey())
            {
                ptr = ptr->right;
            }
            if (ptr->getKey() > min && ptr->getKey() > max)
            {
                ptr = ptr->left;
            }
