...
public:
    ...
    int maximalSum()
    {
        ...
        while (aux)
        {
            if (aux->left == nullptr && aux->right == nullptr) // Foglia
            {
                BSTNode<T> *x = aux;
                somma = 0;
                while (x)
    ...
    void oddPrune()
    {
        return oddPrune(root);
    }

    void oddPrune(BSTNode<T> *ptr)
    {
        if (ptr == nullptr)
        {
            return;
    ...
    void cancellaNodo(BSTNode<T> *ptr)
    {
        if (ptr->left)
        {
            int key = ptr->left->key;
            if (key % 2 == 0)
            {
                ptr->left = nullptr;
            }
